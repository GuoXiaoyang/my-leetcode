...
var groupAnagrams = function(strs) {
  ...
  strs.sort();
  var result=[],tmp=[],resultDict={};


  for(var i = 0;i<strs.length;i++) {
    var tmpStr=sortString(strs[i]);
    console.log("tmpStr:",tmpStr);
    if(resultDict[tmpStr] !== undefined) {
      resultDict[tmpStr].push(strs[i]);
    } else {
      resultDict[tmpStr] = [];
      resultDict[tmpStr].push(strs[i]);
    }
  ...
  for(var ele in resultDict) {
    result.push(resultDict[ele]);
  }
  return result;
};
