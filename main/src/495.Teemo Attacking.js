...
var findPoisonedDuration = function(timeSeries, duration) {
  var cnt = timeSeries.length;
  var total = cnt*duration;
  for(var i=1;i<cnt;i++) {
    total-=(timeSeries[i]-timeSeries[i-1]>duration?0:duration-timeSeries[i]+timeSeries[i-1]);
  }
  return total;
};

//test
var arr = [1,4], duration = 2;
var algo = "algo";
console.time(algo);
var res = findPoisonedDuration(arr,duration);
