...
/****************************************************************
 ...
 path and file infos are separated by a single blank space.
...
var findDuplicate = function(paths) {
  var res=[],map={};
  var dir1,dir2,content;
  for(var i=0;i<paths.length;i++) {
    var files=paths[i].split(' ');
    dir1=files[0];
    for(var j=1;j<files.length;j++) {
      dir2=files[j].substring(0,files[j].indexOf('('));
      content=files[j].substring(files[j].indexOf('(')+1,files[j].indexOf(')'));
      if(!map[content]) map[content]=[];
      map[content].push(dir1+'/'+dir2);
    }
  }
  for(var item in map) {
    if(map[item].length>1) res.push(map[item]);
  }
  return res;
};
